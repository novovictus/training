...
function renderQuestion(){
  ...
  $('question-count').textContent=`Question ${index+1} of ${active.items.length} · Domain ${q.domain} · Mastery ${Math.min(masteryFor(q.id).correct,3)}/3`;
...
function submit(expired){
  ...
  const items=active.items.map((runtime,number)=>{const q=questionById(runtime.questionId),r=active.responses[q.id],correct=r.answer===q.answer;const m=masteryFor(q.id);state.mastery[q.id]={attempts:m.attempts+1,correct:m.correct+(correct?1:0),mastered:m.mastered||(correct&&m.correct+1>=3),lastAttempt:finishedAt};return{...r,id:q.id,number:number+1,domain:q.domain,target:q.target,stem:q.stem,correct,correctAnswer:q.answer,optionOrder:runtime.optionOrder};});
...
function renderResults(result){
  showView('results-view');const unanswered=result.items.filter(x=>!x.answer).length;const newlyMastered=result.items.filter(x=>state.mastery[x.id].mastered&&state.mastery[x.id].correct===3).length;
